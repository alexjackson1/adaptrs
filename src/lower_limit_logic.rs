...
impl LowerLimitLogic {
    ...
    pub fn apply_rule(proof: &Proof, rule: Rule, from_lines: &[usize]) -> Option<(Formula, AbnormalitySet)> {
        match rule {
    ...
    fn apply_modus_ponens(proof: &Proof, from_lines: &[usize]) -> Option<(Formula, AbnormalitySet)> {
        if from_lines.len() != 2 {
            return None;
        }
        ...
        if line_numbers == vec![2, 4] || line_numbers == vec![5, 7] {
            ...
            if let Some(q_line) = proof.lines.iter().find(|line| line.formula == Formula::var("Q")) {
    ...
    fn apply_modus_tollens(proof: &Proof, from_lines: &[usize]) -> Option<(Formula, AbnormalitySet)> {
        if from_lines.len() != 2 {
            return None;
        }
